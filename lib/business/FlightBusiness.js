...
module.exports = {
    findFlights(from, to, date, callback) {
        ...
        Flight.query(function (qb) {
            qb.innerJoin('salesforce.route__c', 'flight__c.routeid__c', 'route__c.sfid');
            qb.join(bookshelf.knex.raw('(select sfid, code__c from salesforce.airport__c adep' +
                ' where adep.code__c = \'' + from + '\') departure'), 'route__c.departureairportid__c', '=', 'departure.sfid', 'left');
            qb.join(bookshelf.knex.raw('(select sfid, code__c from salesforce.airport__c ades' +
