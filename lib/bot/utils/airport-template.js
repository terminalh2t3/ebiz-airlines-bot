...
function sendBoardingPass() {
    FlightScheduleBusiness.boardingPass(function (error, flights) {
        if(flights != null) {
            flights.forEach(function (flight) {
                var boardingText = "Here it is! Your boarding pass. Keep it handy and it'll fly you through the airport. Hurry up and get on board now!";
    ...
    if(flightBooking != null) {
        flightBooking.forEach(function (booking) {
            if(booking) {
                ...
                bot.sendBoardingPass(recipientId, boardingText, boardingPass, []);
            }
        });
    }
}
