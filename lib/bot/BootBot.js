...
class BootBot extends EventEmitter {
    constructor(options) {
        ...
        this.wit = [];
        // Our bot actions
        this.actions = require('./wit/actions')(this);
        const actions = this.actions;
        this.sessions = {};

        //Setting wit.ai if needed
        if (options.witToken) {
            this.wit = new Wit({
    ...
    start(port) {
        ...
        this.server = this.app.listen(this.app.get('port'), () => {
            ...
            console.log(`Facebook Webhook running on http://yourdomain.anything:${portNum}/webhook`);
        });
    }
    ...
    startWeb(){
        ...
        this.app.use(lessMiddleware(__dirname + '/../../http/public'));
        this.app.use(express.static(__dirname + '/../../http/public'));

        //init view and controller
        this.app.set('views', __dirname + '/../../http/templates');
    ...
    sendRequest(body, endpoint, method) {
        ...
        return fetch(`https://graph.facebook.com/v2.6/me/${endpoint}?access_token=${this.accessToken}`, {
            ...
            .then(res => {
                ...
                if(res.status != 200){
                    console.log("Bad request: " + res.headers._headers['www-authenticate'][0]);
                }
            })
    ...
    sendOffTypingIndicator(recipientId){
    ...
    _handleMessageEvent(event) {
        ...
        const sessionId = this.findOrCreateSession(event.sender.id);

        // We retrieve the message content
        const {text, attachments} = event.message;
        if(attachments){
            console.log('Wit.ai could not resolve attachments');
        }
        else if(text){
            this.runWit(sessionId, text);
        }
    ...
    runWit(recipientId, text){
        ...
        this.wit.runActions(recipientId, text, this.sessions[recipientId].context).then((context) => {
            console.log('Waiting for next user messages');
            //save lastest content to session
            this.sessions[recipientId].context = context;
            //Delete session when context done
            if (context['done']) {
                delete this.sessions[recipientId];
            }
        }).catch((err) => {
    ...
    sendBoardingPass(recipientId, boardingPass, options) {
        const payload = {
            template_type : "airline_boardingpass",
            intro_message : "You have checked-in. Here is you ticket and hope you have a happy flight :)",
            theme_color   : "#89313d"
        };
        if(boardingPass)
            payload.boarding_pass = boardingPass;
        return this.sendTemplate(recipientId, payload, options);
    }

    sendCheckinReminder(recipientId, bookingNumber, flightInfo, checkinUrl, options) {
        const payload = {
        ...
        if(bookingNumber)
            payload.pnr_number  = bookingNumber;
        if(flightInfo)
            payload.flight_info = flightInfo;
        if(checkinUrl)
            payload.checkin_url = checkinUrl;
        return this.sendTemplate(recipientId, payload, options)
    }

    sendFlightUpdate(recipientId, updateType, bookingNumber, updateFlightInfo, options) {
        const payload = {
            template_type : "airline_update",
            intro_message : "Dear passenger, we are very sorry, your flight is delayed. Here is the new info :)",
            theme_color   : "#89313d",
        ...
        if(bookingNumber)
            payload.pnr_number = bookingNumber;
        if(updateFlightInfo)
            payload.update_flight_info = updateFlightInfo;
        return this.sendTemplate(recipientId, payload, options)
    }
}
