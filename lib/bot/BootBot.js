...
class BootBot extends EventEmitter {
    ...
    runWit(recipientId, text){
        const sessionId = this.findOrCreateSession(recipientId);
    ...
    _initWebhook() {
        this.app.get('/webhook', (req, res) => {
            if (req.query['hub.mode'] === 'subscribe' && req.query['hub.verify_token'] === this.verifyToken) {
                ...
                console.error('Failed validation. Make sure the validation tokens match. True token is: ' + this.verifyToken);
                res.sendStatus(403);
            }
        });
    ...
    _verifyRequestSignature(req, res, buf) {
        var sfcallout = req.headers['x-sf-callouts'];
        if(sfcallout){
