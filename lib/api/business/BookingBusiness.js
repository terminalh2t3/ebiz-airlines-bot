...
const FlightSchedule = require('../models/Flight');
...
const listFunctions = {
    ...
    getBookingDetail(passengerId, flightId, callback){
        Booking.where({passenger_id: passengerId, flight_id: flightId}).fetch({
    ...
    getCheckInDetail(bookingSfId, passengerSfId, callback) {
        Booking.query(function (qb) {
            ...
            withRelated: ['flightSchedule', 'passenger', 'flightSchedule.route','flightSchedule.route.destination','flightSchedule.route.departure']
        }).then(function(model){
            console.log(model);
            callback(null, model ? model.toJSON() : null)
        }).catch(function(e){
    ...
    validateBooking(passengerId, flightId, callback){
        Booking.where({passenger_id: passengerId, flight_id: flightId})
            .count('booking_id')
            .then(function(count){
                if(count > 0){
                    callback(null, false);
